...
class Contenido(Scoring): 
    """
    Subclase del objeto "Scoring". Se encarga de realizar los cálculos 
    relacionados con la recomendación en base al contenido. Solo es aplicable
    con el tipo de dataset "Movies".
    """
    ...
    def inicialitzar(self, dataset: Conjuntos) -> None: 
        """
        Esta función inicializa la matriz TFIDF necesaria para poder 
        hacer recomendaciones en base al contenido. 
          
        Parameters
        ----------
        dataset: Conjuntos
            Un objeto de la clase conjuntos que contiene una matriz de 
            valoraciones y elementos.
        ...
        logging.debug("Vocabulario ({}): {}".format(len(tfidf.get_feature_names_out()), 
    ...
    def calcular_perfil(self, fila_user: np.ndarray) -> np.ndarray: 
        ...
        -------
        np.ndarray
            El array del perfil del usuario, donde asigna la importancia del 
            usuario respecto a cada una de las características del vocabulario 
            de conjunto de dartos. 
    ...
    def calcular_distancia_cosinus(self, perfil_usuario: np.ndarray) -> np.ndarray: 
        ...
        -------
            distancia_items : np.ndarray
                Devuelve un arrray donde cada valor es la distancia entre las 
                valoraciones del usuario y los items. 
        ...
        distancias_items = similitud_items/np.sqrt((perfil_usuario**2).sum()*
                                                   (self._tfidf_matrix**2).sum(axis=1))
